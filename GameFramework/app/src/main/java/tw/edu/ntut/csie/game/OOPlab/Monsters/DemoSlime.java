...
public class DemoSlime extends Monster implements GameObject {
    private Animation _slime_Stand;
    ...
    private final int _maxHP = 999, _maxMP = 0;
    private int _HP, _MP;
    private final int _EXPvalue = 100;
    private boolean _ExpAdded;


    private static final int _DMG = 0;//攻擊力
    private static final double _HITr = 0;//命中率
    private static final double _FLEEr = 0;//迴避率
    private static final double _CRT = 0;//爆擊倍率
    private static final double _CRTr = 0;//爆擊機率
    private static final int _DEF = 0;//防禦力

    private static final boolean _initiative = false; //主動攻擊
    ...
    private static final int _rebornTime = 100;//小於零表示不會重生
    ...
    public void initialize(Hero hero) {
        ...
        _slime_Stand = new Animation();
        _slime_Stand.addFrame(R.drawable.slime_stand1);
        _slime_Stand.addFrame(R.drawable.slime_stand2);
        _slime_Stand.addFrame(R.drawable.slime_stand3);
        _slime_Stand.setDelay(6);
        ...
        _HP = _maxHP;
        _MP = _maxMP;
        _dpHP = new Integer(3, _HP, 0, 0 - 25);
...
//        跟地圖一起消失或顯示
        if (_locatedMap.checkVisible()) {
            ...
            _slime_Stand.setLocation(_spx, _spy);
...
//        int director = (int) (Math.random() * 80);
    ...
    public int checkWidth() {
        return _slime_Stand.getWidth();
    }
    public int checkHeight() {
        return _slime_Stand.getHeight();
    }
    public int checkMpx() {
        return _mpx;
    }
